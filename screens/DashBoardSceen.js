...
function DashBoardSceen({ navigation, todos }) {
    console.log("할일 목록 = 통계 : " , todos.length)
    const groupedByCategory = todos.reduce((groupedByCategory, todo) => {
        if(!groupedByCategory[todo.category])
            groupedByCategory[todo.category] = 0
    ...
    for(let category in groupedByCategory) {
        ...
        data.push({value : groupedByCategory[category], frontColor : '#006DFF' , 
        gradientColor : '#009FFF' , spacing : 6 , label : category})
    }
    console.log(data)
    ...
    const yAxisLabelTexts = Array(noOfSections).fill(0).map((_, id) => {
        ...
        if(unit > 1000000) {
            unit = (unit / 1000000).toString() + 'M'
        } else if(unit > 1000) {
            unit = (unit / 1000).toString() + 'K'
        }
        return unit
    })
